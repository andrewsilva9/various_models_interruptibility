...
def draw_int_timeline(image, int_list, starting_line=0, box_width=3, box_height=3, label='unknown'):
...
def draw_timeline(annotator_name):
    for trial_id in range(1, 16):
        ...
        for participant in annotations.keys():
            ...
            box_height = 8
            box_width = 4
            img = np.ones((box_height*21, len(part_ground)*box_width + 200, 3), np.uint8)
            img *= 255
